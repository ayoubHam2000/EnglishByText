...
class D_ask_color(context: Context, val event: (Int) -> Unit) : MyDialogBuilder(
    context,
    R.layout.d_pick_color
) {
    ...
    private fun initColor(){
        ...
        if(theIntColor != -1){
            val r = Color.red(theIntColor)
            val g = Color.green(theIntColor)
            val b = Color.blue(theIntColor)

            redController.progress = r
            greenController.progress = g
            blueController.progress = b
